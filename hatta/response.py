...
def response(request, title, content, etag='', mime='text/html', rev=None, size=None, date=None):
    """
...
class WikiResponse(Response):
    ...
    def make_conditional(self, request):
        # Default pages have an etag that ends with -1 since
        # these are static files, add an old modified date.
